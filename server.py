...
def home():
    ...
    # Get the last 7 days of actual stock prices and the predicted price for the next day
    #last_7_days = scaler.inverse_transform(data[-7:])

    last_7_days = []
    for i in range(-7, 0):
        last_7_days.append([dates[i], scaler.inverse_transform([data[i]])[0][0]])

    next_day_predicted = predicted[-1][0]

    # Render the template and pass the data to it
    return render_template('home.html', symbol=symbol, last_7_days=last_7_days, next_day_predicted=next_day_predicted)
...
def prediction():
    if request.method == 'POST':
        # Get the user input
        user_input = np.array(request.form.getlist('user_input'), dtype=np.float32)
        ...
        # Render the template and pass the data to it
        return render_template('prediction.html', user_input=request.form.getlist('user_input'), next_day_predicted=next_day_predicted)

    # Render the template without any data if the user hasn't submitted the form yet
